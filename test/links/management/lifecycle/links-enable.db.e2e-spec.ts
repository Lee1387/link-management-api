...
describe('Links Enable (db e2e)', () => {
  ...
  it('PATCH /links/:id/enable should enable the authenticated user owned link', async () => {
    ...
    const createdLink = await createOwnedLink(
      ...
      'https://example.com/articles/enable-me',
    );
    createdLinkIds.add(createdLink.id);

    await disableOwnedLink(app, loginBody.accessToken, createdLink.id);
    const enabledLink = await enableOwnedLink(
      app,
      loginBody.accessToken,
      createdLink.id,
    );

    expect(enabledLink.id).toBe(createdLink.id);
    expect(enabledLink.disabledAt).toBeNull();
    ...
    expect(persistedLink?.disabledAt).toBeNull();
  });

  it('PATCH /links/:id/enable should return not found for another user link', async () => {
    app = await createLinksDbApp();
    ...
    await disableOwnedLink(app, ownerLoginBody.accessToken, ownerLink.id);

    const response = await app.inject({
      method: 'PATCH',
      url: `/links/${ownerLink.id}/enable`,
      headers: {
